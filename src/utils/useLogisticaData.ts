...
import { LogisticaRecord, Cliente, Transportadora, FolhaObra, Armazem } from '@/types/logistica';
...
export function useLogisticaData() {
  ...
  const updateCache = useCallback((dateStr: string, data: LogisticaRecord[]) => {
  ...
  // Fetch logistics data for a specific date - filtering by data in logistica_entregas
  const fetchLogisticaData = useCallback(async (date: Date | null) => {
    if (!date) return [];
    setLoading(true);
    ...
    try {
      ...
      if (data) {
        ...
        const unique = Array.from(
          new Map(data.map((item: LogisticaRecord) => [item.id, item])).values()
        ) as LogisticaRecord[];
    ...
    const fetchWithRetry = async () => {
      try {
        ...
        if (transportadorasData) {
          setTransportadoras(transportadorasData.map((t: any) => ({ 
            value: t.id, 
            label: t.name 
          })));
        }
        ...
        if (armazensData) {
          setArmazens(armazensData.map((a: any) => ({
            value: a.id,
            label: a.nome_arm,
            morada: a.morada,
        ...
        console.error(`Error fetching reference data (attempt ${retries + 1}):`, error);
  ...
  const refetchRow = useCallback(async (rowId: string, currentDate: Date | null) => {
    try {
      ...
      if (data) {
        ...
        if (!processedRow.items_base) {
          processedRow.items_base = currentRow?.items_base || { id: '', descricao: '', brindes: false };
        }
        
        // Preserve item description if it exists in current data but is missing in fetched data
        if (currentRow?.items_base?.descricao && !processedRow.items_base.descricao) {
        ...
        if (!processedRow.items_base.folhas_obras) {
          processedRow.items_base.folhas_obras = currentRow?.items_base?.folhas_obras || { 
            id: '', 
            numero_orc: '', 
            numero_fo: '',
            cliente: '',
  ...
  const updateLogisticaField = useCallback(async (
    rowId: string, 
    field: string, 
    value: any,
    ...
    try {
      ...
      if (field === 'guia') {
        ...
        console.log(`Updating guia field with value:`, { rowId, original: value, processed: processedValue });
      }
  ...
  const updateFolhaObraField = useCallback(async (
    folhaObraId: string, 
    field: string, 
    value: any,
    currentDate: Date | null
    ...
    try {
      ...
      if (field === 'numero_orc' && (value === '' || value === null || value === undefined)) {
        processedValue = null;
      ...
      setRecords(prevRecords => 
        prevRecords.map(record => {
          if (record.items_base?.folha_obra_id === folhaObraId || 
      ...
      if (currentDate) {
        const dateStr = format(currentDate, 'yyyy-MM-dd');
        if (dataCacheRef.current[dateStr]) {
          dataCacheRef.current[dateStr] = dataCacheRef.current[dateStr].map(record => {
            if (record.items_base?.folha_obra_id === folhaObraId || 
                record.items_base?.folhas_obras?.id === folhaObraId) {
              // Create a deep clone to avoid reference issues
              const updatedRecord = JSON.parse(JSON.stringify(record));
  ...
  const updateItemBaseField = useCallback(async (
    itemId: string, 
    field: string, 
    value: any,
    currentDate: Date | null
  ...
  // Delete a row
  const deleteLogisticaRow = useCallback(async (rowId: string, currentDate: Date | null) => {
    try {
      const { error } = await supabase
        .from('logistica_entregas')
        .delete()
      ...
      if (currentDate) {
        const dateStr = format(currentDate, 'yyyy-MM-dd');
        if (dataCacheRef.current[dateStr]) {
          dataCacheRef.current[dateStr] = dataCacheRef.current[dateStr].filter(
            r => r.id !== rowId
          );
        }
      }
  ...
  const duplicateLogisticaRow = useCallback(async (row: LogisticaRecord, currentDate: Date | null) => {
    try {
      ...
      const newLogisticaData = {
        ...
        quantidade: row.quantidade !== null ? row.quantidade : (row.items_base?.quantidade || null),
        notas: row.notas,
        ...
        data: currentDate ? format(currentDate, 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd'),
        is_entrega: true
      };
