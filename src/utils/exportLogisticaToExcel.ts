...
export const exportLogisticaToExcel = ({
  filteredRecords,
  selectedDate,
  clientes = [],
  armazens = [],
  ...
  sortedRecords.forEach((row, idx) => {
    ...
    if (row.contacto_entrega)
      parts.push('Cont. Entreg.\n' + row.contacto_entrega)
    if (row.telefone_entrega)
      parts.push('Tel. Entreg.\n' + row.telefone_entrega)
    ...
    const descricaoColIdx =
      columns.findIndex((col) => col.key === 'descricao') + 1
    const clienteColIdx =
      columns.findIndex((col) => col.key === 'id_cliente') + 1
    const notasColIdx = columns.findIndex((col) => col.key === 'notas') + 1
    const localRecolhaColIdx =
      columns.findIndex((col) => col.key === 'local_recolha') + 1
    const localEntregaColIdx =
      columns.findIndex((col) => col.key === 'local_entrega') + 1
    if (descricaoColIdx > 0)
      excelRow.getCell(descricaoColIdx).alignment = {
        ...excelRow.getCell(descricaoColIdx).alignment,
        wrapText: true,
        vertical: 'top',
      }
    ...
    if (localRecolhaColIdx > 0)
      excelRow.getCell(localRecolhaColIdx).alignment = {
        ...excelRow.getCell(localRecolhaColIdx).alignment,
        wrapText: true,
        vertical: 'top',
      }
    if (localEntregaColIdx > 0)
      excelRow.getCell(localEntregaColIdx).alignment = {
        ...excelRow.getCell(localEntregaColIdx).alignment,
        wrapText: true,
        vertical: 'top',
      }
  ...
  workbook.xlsx.writeBuffer().then((buffer) => {
    saveAs(
      new Blob([buffer]),
      `logistica_${selectedDate ? selectedDate.toISOString().slice(0, 10) : 'all'}.xlsx`,
    )
  })
}
