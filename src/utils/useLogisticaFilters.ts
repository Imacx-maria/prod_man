...
export function useLogisticaFilters(
  ...
  const applyFilters = useCallback((
    ...
    if (!Object.values(filters).some(value => value && value.trim())) return records;
    ...
    return records.filter(row => {
      ...
      return preparedFilters.every(({ field, value }) => {
        ...
        switch (field) {
          case 'orc':
            return String(row.items_base?.folhas_obras?.numero_orc || '').toLowerCase().includes(value);
          case 'fo':
            return String(row.items_base?.folhas_obras?.numero_fo || '').toLowerCase().includes(value);
          case 'guia':
            return String(row.guia || '').toLowerCase().includes(value);
          case 'tipo':
            return (row.items_base?.brindes ? 'Brindes' : 'Print').toLowerCase().includes(value);
          case 'cliente': {
            const clientId = row.items_base?.folhas_obras?.id_cliente;
            const clientName = clientId ? clienteLookup[clientId] || row.items_base?.folhas_obras?.cliente || '' : '';
            return clientName.toLowerCase().includes(value);
          }
          case 'item':
            return String(row.items_base?.descricao || '').toLowerCase().includes(value);
          case 'local_recolha': {
            const recolhaId = row.id_local_recolha;
            const recolhaName = recolhaId ? clienteLookup[recolhaId] || row.local_recolha || '' : '';
            return recolhaName.toLowerCase().includes(value);
          }
          case 'local_entrega': {
            const entregaId = row.id_local_entrega;
            const entregaName = entregaId ? clienteLookup[entregaId] || row.local_entrega || '' : '';
            return entregaName.toLowerCase().includes(value);
          }
          case 'transportadora':
            return String(row.transportadora || '').toLowerCase().includes(value);
          case 'notas':
            return String(row.notas || '').toLowerCase().includes(value);
          case 'saiu': {
            // For checkboxes, we filter by "sim" or "não"
            const isSaiu = row.items_base?.folhas_obras?.saiu || row.saiu || false;
            if (value === 'sim' || value === 's') return isSaiu;
            if (value === 'não' || value === 'nao' || value === 'n') return !isSaiu;
            return true;
          }
          default:
            return true;
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (!inputsChanged && 'cachedResult' in prevInputsRef.current) {
      return (prevInputsRef.current as any).cachedResult;
    }
    
    // Apply filters and cache result
    const result = applyFilters(records, filters, clienteLookup);
    (prevInputsRef.current as any).cachedResult = result;
    return result;
