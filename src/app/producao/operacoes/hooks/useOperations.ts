...
export const useOperations = () => {
  const [operations, setOperations] = useState<ProductionOperationWithRelations[]>([])
  ...
  const fetchOperations = useCallback(async () => {
    try {
      ...
      let query = supabase
        ...
        .select(`
          *,
          folhas_obras!inner(numero_fo, nome_campanha, cliente),
      ...
      if (data) {
        ...
        if (itemIds.length > 0) {
          const { data: operacoesData } = await supabase
            ...
            .select('item_id, operador_id, maquina, material_id, num_placas_print, num_placas_corte, observacoes, no_interno')
            .in('item_id', itemIds)
  ...
  const createOperation = useCallback(async (operationData: ProductionOperationInput) => {
    try {
      ...
      if (stockData.quantidade_disponivel < operationData.quantidade_material_usado) {
        throw new Error(`Estoque insuficiente. Disponível: ${stockData.quantidade_disponivel}`)
      }
      ...
      const newAvailableQuantity = stockData.quantidade_disponivel - operationData.quantidade_material_usado
      
      const { data: stockConsumption, error: stockConsumptionError } = await supabase
        .from('stocks')
        .insert([{
      ...
      if (stockConsumptionError) {
        ...
        await supabase.from('producao_operacoes').delete().eq('id', operationResult.id)
        throw new Error(`Erro ao consumir estoque: ${stockConsumptionError.message}`)
      }
  ...
  // Update operation
  const updateOperation = useCallback(async (id: string, updates: Partial<ProductionOperationInput>) => {
    try {
  ...
  const completeOperation = useCallback(async (id: string) => {
    try {
      ...
      setOperations(prev => prev.map(op => 
        op.id === id 
          ? { ...op, concluido: true, status: 'Concluído', data_conclusao: data.data_conclusao, hora_fim: data.hora_fim }
          : op
      ))
