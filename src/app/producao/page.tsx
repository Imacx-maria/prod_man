...
import { ArrowLeft, ArrowRight, Grid2x2Check, Loader2, SquareChartGantt, FileSpreadsheet } from 'lucide-react';
...
const JobsTableSkeleton = () => (
  <div className="space-y-2">
    <div className="h-10 bg-gray-200 rounded animate-pulse" /> {/* Header */}
    {[...Array(8)].map((_, i) => (
...
export default function ProducaoPage() {
  ...
  const [jobsCompletionStatus, setJobsCompletionStatus] = useState<Record<string, { completed: boolean; percentage: number }>>({});
  ...
  /* tab state */
  const [activeTab, setActiveTab] = useState<'em_curso' | 'concluidos'>('em_curso');
  ...
  /* sorting */
  type SortableJobKey = 'numero_orc' | 'numero_fo' | 'cliente' | 'nome_campanha' | 'notas' | 'prioridade' | 'data_concluido' | 'concluido' | 'saiu' | 'fatura' | 'artwork';
  const [sortCol, setSortCol] = useState<SortableJobKey>('prioridade');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc');
  const toggleSort = useCallback((c: SortableJobKey) => {
  ...
  const checkOrcDuplicate = useCallback(async (orcNumber: number, currentJobId: string) => {
    ...
    try {
      ...
      if (currentJobId && currentJobId.trim() !== '' && !currentJobId.startsWith('temp-')) {
  ...
  const checkFoDuplicate = useCallback(async (foNumber: string, currentJobId: string) => {
  ...
  const fetchJobs = useCallback(async (page = 0, reset = false, filters: {
    ...
    try {
      ...
      if (filters.activeTab === 'em_curso') {
        ...
        query = query.or(`data_concluido.gte.${twoMonthsAgoString},updated_at.gte.${twoMonthsAgoString},created_at.gte.${twoMonthsAgoString}`);
      }
      ...
      if ((filters.itemF && filters.itemF.trim() !== '') || (filters.codeF && filters.codeF.trim() !== '')) {
        if (filteredJobs.length > 0) {
          ...
          if (filters.itemF && filters.itemF.trim() !== '') {
            itemQuery = itemQuery.ilike('descricao', `%${filters.itemF.trim()}%`);
          }
          ...
          if (matchingItems) {
            const matchingJobIds = new Set(matchingItems.map((item: any) => item.folha_obra_id));
            filteredJobs = filteredJobs.filter(job => matchingJobIds.has(job.id));
          } else {
            filteredJobs = [];
          }
        }
      ...
      setError(`Failed to load production jobs: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const fetchJobsSaiuStatus = useCallback(async (jobIds: string[]) => {
    ...
    try {
      ...
      if (itemsData && itemsData.length > 0) {
        ...
        jobIds.forEach(jobId => {
          const jobItems = itemsData.filter(item => item.folha_obra_id === jobId);
          ...
          const allItemsSaiu = jobItems.every(item => {
            const logisticsEntry = logisticsData?.find(l => l.item_id === item.id);
  ...
  const fetchJobsCompletionStatus = useCallback(async (jobIds: string[]) => {
    ...
    try {
      ...
      if (itemsData && itemsData.length > 0) {
        ...
        const jobCompletionStatus: Record<string, { completed: boolean; percentage: number }> = {};
        ...
        jobIds.forEach(jobId => {
          ...
          // Calculate completion percentage based on logistics entries
          const completedItems = jobItems.filter(item => {
            const logisticsEntry = logisticsData?.find(l => l.item_id === item.id);
            return logisticsEntry && logisticsEntry.concluido === true;
          });
          
          const percentage = Math.round((completedItems.length / jobItems.length) * 100);
  ...
  const fetchItems = useCallback(async (jobIds: string[]) => {
    if (jobIds.length === 0) return;
    ...
    try {
      ...
      if (itemsData) {
        // Merge designer data with items data
        ...
        setAllItems(prev => {
          ...
          const filtered = prev.filter(item => !jobIds.includes(item.folha_obra_id));
  ...
  useEffect(() => {
    if (debouncedFoF || debouncedCampF || debouncedItemF || debouncedCodeF || debouncedClientF || showFatura) {
  ...
  }, [debouncedFoF, debouncedCampF, debouncedItemF, debouncedCodeF, debouncedClientF, showFatura, activeTab, fetchJobs]);

  // Load items when jobs change
  useEffect(() => {
    if (jobs.length > 0) {
      const jobIds = jobs.map(job => job.id).filter(id => !id.startsWith('temp-'));
      if (jobIds.length > 0) {
        fetchItems(jobIds);
  ...
  }, [loading.jobs, hasMoreJobs, currentPage, fetchJobs, debouncedFoF, debouncedCampF, debouncedItemF, debouncedCodeF, debouncedClientF, showFatura, activeTab]);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <Input placeholder="Filtra FO"        className="rounded-none w-28" value={foF}    onChange={(e)=>setFoF(e.target.value)} />
          <Input placeholder="Filtra Nome Campanha" className="rounded-none flex-1" value={campF}  onChange={(e)=>setCampF(e.target.value)} />
          <Input placeholder="Filtra Item"      className="rounded-none flex-1" value={itemF} onChange={(e)=>setItemF(e.target.value)} />
          <Input placeholder="Filtra Código"    className="rounded-none w-40" value={codeF}  onChange={(e)=>setCodeF(e.target.value)} />
          <Input placeholder="Filtra Cliente"   className="rounded-none flex-1" value={clientF}onChange={(e)=>setClientF(e.target.value)} />

          <Button size="icon" variant="outline" onClick={()=>{
            setFoF(""); setCampF(""); setItemF(""); setCodeF(""); setClientF("");
          }}>
            <X className="w-4 h-4" />
          </Button>
          <Button size="icon" variant={showFatura ? "default" : "outline"} onClick={() => setShowFatura((v) => !v)} title={showFatura ? "Mostrar não faturados" : "Mostrar só faturados"}>
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const jobIds = sorted.map(job => job.id).filter(id => !id.startsWith('temp-'));
                      ...
                      console.log(`Exporting data for ${jobIds.length} jobs with incomplete logistics...`);
                      ...
                      const transportadorasMap = new Map(
                        (transportadorasData || []).map((t: any) => [t.id, t.name])
                      );

                      // Fetch items with logistics data for these jobs (only incomplete logistics)
                      const { data: itemsWithLogistics, error } = await supabase
                        .from('items_base')
                        .select(`
                          id,
                          folha_obra_id,
                      ...
                      if (!itemsWithLogistics || itemsWithLogistics.length === 0) {
                        alert('Não há itens com logística incompleta (concluído=false) para exportar.');
                        return;
                      }

                      // Transform data for export
                      const exportRows = (itemsWithLogistics || []).map((item: any) => {
                        // Get the latest logistics entry for this item (if multiple exist)
                        const latestLogistics = item.logistica_entregas && item.logistica_entregas.length > 0 
                          ? item.logistica_entregas.reduce((latest: any, current: any) => {
                              if (!latest) return current;
                              if (!current.data_concluido) return latest;
                              if (!latest.data_concluido) return current;
                              return new Date(current.data_concluido) > new Date(latest.data_concluido) ? current : latest;
                            }, null)
                          : null;

                        // Resolve transportadora ID to name
                        const transportadoraName = latestLogistics?.transportadora 
                          ? transportadorasMap.get(latestLogistics.transportadora) || latestLogistics.transportadora
                          : '';
                        ...
                        return {
                          ...
                          quantidade: item.quantidade || null,
                          nome_campanha: item.folhas_obras?.nome_campanha || '',
                          descricao: item.descricao || '',
                          data_in: item.folhas_obras?.data_in || null,
                          data_concluido: latestLogistics?.data_concluido || null,
                          transportadora: transportadoraName,
                          local_entrega: latestLogistics?.local_entrega || ''
                        };
                      });
      ...
      {/* Main Tabs */}
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'em_curso' | 'concluidos')} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="em_curso">Em Curso ({activeTab === 'em_curso' ? jobs.length : '...'})</TabsTrigger>
          <TabsTrigger value="concluidos">Concluídos ({activeTab === 'concluidos' ? jobs.length : '...'})</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="em_curso">
              ...
              <Table className="w-full border-0">
                <TableHeader>
                  <TableRow className="rounded-none">
                    <TableHead onClick={()=>toggleSort('numero_orc')} className="sticky top-0 z-10 cursor-pointer select-none w-[100px] max-w-[100px] text-ellipsis overflow-hidden whitespace-nowrap bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border text-right">
                      ORC {sortCol==='numero_orc' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>)}
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('numero_fo')} className="sticky top-0 z-10 cursor-pointer select-none w-[100px] max-w-[100px] text-ellipsis overflow-hidden whitespace-nowrap bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border text-right">
                      FO {sortCol==='numero_fo' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>)}
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('cliente')} className="sticky top-0 z-10 cursor-pointer select-none w-[200px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      Cliente {sortCol==='cliente' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>)}
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('nome_campanha')} className="sticky top-0 z-10 cursor-pointer select-none flex-1 bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                    ...
                    <TableHead onClick={()=>toggleSort('notas')} className="sticky top-0 z-10 cursor-pointer select-none w-[50px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      Nota {sortCol==='notas' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>) }
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('prioridade')} className="sticky top-0 z-10 cursor-pointer select-none w-[210px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      Status {sortCol==='prioridade' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>) }
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('concluido')} className="sticky top-0 z-10 cursor-pointer select-none text-center p-0 w-[36px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span>C {sortCol==='concluido' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>) }</span>
                          </TooltipTrigger>
                          <TooltipContent>Prod. Concluída</TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </TableHead>

                    <TableHead onClick={()=>toggleSort('prioridade')} className="sticky top-0 z-10 cursor-pointer select-none text-center p-0 w-[36px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span>P {sortCol==='prioridade' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>) }</span>
                          </TooltipTrigger>
                          <TooltipContent>Prioridade</TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('artwork')} className="sticky top-0 z-10 cursor-pointer select-none text-center p-0 w-[36px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span>A {sortCol==='artwork' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>)}</span>
                          </TooltipTrigger>
                    ...
                    <TableHead className="sticky top-0 z-10 w-[100px] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      Ações
                    </TableHead>
                    <TableHead onClick={()=>toggleSort('fatura')} className="sticky top-0 z-10 cursor-pointer select-none text-center p-0 w-[9ch] max-w-[9ch] bg-[var(--orange)] text-black uppercase rounded-none border-b-2 border-border">
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span>F {sortCol==='fatura' && (sortDir==='asc'?<ArrowUp className="inline w-3 h-3 ml-1"/>:<ArrowDown className="inline w-3 h-3 ml-1"/>)}</span>
                          </TooltipTrigger>
                          <TooltipContent>Faturação</TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </TableHead>
                  </TableRow>
          ...
          <TableBody>
            {sorted.map(job=>{
              const its = allItems.filter(i=>i.folha_obra_id===job.id);
              ...
              return (
                <TableRow key={job.id} className="hover:bg-[var(--main)]">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setJobs(prevJobs => prevJobs.map(j =>
                            j.id === job.id ? { ...j, numero_orc: value } : j
                          ));
                        }}
                        onBlur={async (e) => {
                          ...
                          if (!inputValue || isNaN(Number(inputValue))) {
                            if (!job.id.startsWith('temp-')) {
                              await supabase.from('folhas_obras').update({ numero_orc: value }).eq('id', job.id);
                            }
                            return;
                          }
                          ...
                          const existingJob = await checkOrcDuplicate(numericValue, job.id);
                          ...
                          if (existingJob) {
                            ...
                            setDuplicateDialog({
                              ...
                              onConfirm: async () => {
                                ...
                                if (job.id.startsWith('temp-')) {
                                  ...
                                  try {
                                    const insertData = {
                                      ...
                                      nome_campanha: job.nome_campanha || 'Nova Campanha',
                                ...
                                console.log('Inserting ORC job (no duplicate) with data:', insertData);
                                ...
                                if (error) {
                                  console.error('Insert error details (no duplicate):', error);
                                  throw error;
                                }
                                    
                                    if (!error && newJob) {
                                      setJobs(prevJobs => prevJobs.map(j =>
                                        j.id === job.id ? newJob as Job : j
                                      ));
                                    }
                                  } catch (error) {
                                  ...
                                  await supabase.from('folhas_obras').update({ numero_orc: numericValue }).eq('id', job.id);
                                }
                                setDuplicateDialog({ 
                                  isOpen: false, 
                                  type: 'orc', 
                            ...
                            if (job.id.startsWith('temp-')) {
                              ...
                              try {
                                ...
                                console.log('Inserting ORC job with data:', insertData);
                  ...
                  <TableCell className="w-[100px] max-w-[100px]">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setJobs(prevJobs => prevJobs.map(j =>
                          j.id === job.id ? { ...j, numero_fo: value } : j
                        ));
                      }}
                                              onBlur={async (e) => {
                          ...
                          if (value) {
                            ...
                            if (existingJob) {
                              ...
                              setDuplicateDialog({
                                ...
                                onConfirm: async () => {
                                  // User confirmed to proceed with duplicate
                                  await supabase.from('folhas_obras').update({ numero_fo: value }).eq('id', job.id);
                                  setDuplicateDialog({ 
                                    isOpen: false, 
                                    type: 'fo', 
                                    value: '', 
                  ...
                  <TableCell className="w-[200px]">
                    <CreatableClienteCombobox
                      ...
                      onChange={async (selectedId: string) => {
                        ...
                        // Debug logging
                        console.log(`Job ${job.numero_fo} - selecting cliente: ${selectedId} -> ${selected?.label}`);
                        setJobs(prevJobs => prevJobs.map(j =>
                          j.id === job.id
                            ? { ...j, id_cliente: selectedId, cliente: selected ? selected.label : '' }
                            : j
                        ));
                        // Persist to Supabase if not a temp job
                        if (!job.id.startsWith('temp-')) {
                          await supabase.from("folhas_obras").update({ 
                  ...
                  <TableCell className="flex-1">
                    <Input
                      ...
                      onChange={(e) => {
                        const value = e.target.value;
                        setJobs(prevJobs => prevJobs.map(j =>
                          j.id === job.id ? { ...j, nome_campanha: value } : j
                        ));
                      }}
                      onBlur={async (e) => {
                        const value = e.target.value;
                        ...
                        if (!job.id.startsWith('temp-')) {
                          await supabase.from('folhas_obras').update({ nome_campanha: value }).eq('id', job.id);
                        }
                      }}
                          ...
                          <div>
                            <SimpleNotasPopover
                              ...
                              onSave={async (newNotas) => {
                                ...
                                setJobs((prev: Job[]) => prev.map((j: Job) => j.id === job.id ? { ...j, notas: newNotas } : j));
                              }}
                  ...
                  <TableCell className="text-center p-0 w-[36px]">
                    <Checkbox
                      checked={!!job.concluido}
                      onCheckedChange={async (checked) => {
                        const value = checked === 'indeterminate' ? false : checked;
                        
                        if (value) {
                          // CHECKING the checkbox (marking as completed)
                          // Check if there are any items for this job
                          if (its.length === 0) {
                            alert("Primeiro tem que introduzir Items no trabalho");
                            return;
                          }
                          ...
                          // Mark ALL logistics entries as completed AND set their completion date to today
                          const itemIds = its.map(item => item.id);
                          
                          // Update database for ALL logistics entries for items in this job
                          const { data: logisticsEntries } = await supabase
                            .from('logistica_entregas')
                            .select('id')
                            .in('item_id', itemIds);
                          
                          if (logisticsEntries && logisticsEntries.length > 0) {
                            const updatePromises = logisticsEntries.map(entry =>
                              supabase.from('logistica_entregas').update({ 
                                concluido: true,
                                data_concluido: todayString
                              }).eq('id', entry.id)
                            );
                            ...
                            await fetchJobsCompletionStatus([job.id]);
                          }
                        } else {
                          // UNCHECKING the checkbox (marking as incomplete)
                          // Mark ALL logistics entries as incomplete
                          const itemIds = its.map(item => item.id);
                        ...
                        setJobs((prevJobs) =>
                          prevJobs.map((j) =>
                            j.id === job.id ? { ...j, concluido: value } : j
                          )
                        );
                        
                        // Update job completion status in database
                        if (!job.id.startsWith('temp-')) {
                          await supabase.from("folhas_obras").update({ concluido: value }).eq("id", job.id);
                        }
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell className="text-center p-0 w-[36px]">
                    <button
                      ...
                      title={
                        job.prioridade
                          ? 'Prioritário'
                          : (job.created_at && (Date.now() - new Date(job.created_at).getTime()) / (1000 * 60 * 60 * 24) > 3
                      ...
                      onClick={async () => {
                        ...
                        // Persist to Supabase
                        await supabase.from("folhas_obras").update({ prioridade: newPrioridade }).eq("id", job.id);
                      }}
                    />
                  </TableCell>
                  <TableCell className="text-center p-0 w-[36px]">
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span>
                            <span
                              className={`w-3 h-3 rounded-full mx-auto flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-primary transition-colors ${getAColor(job.id, allItems)}`}
                              title="Tem AF's"
                            />
                          </span>
                        </TooltipTrigger>
                  ...
                  <TableCell className="w-[100px] p-0 pr-2">
                    <div className="flex gap-2 justify-center">
                      <Button size="icon" variant="default" onClick={()=>setOpenId(job.id)}><Eye className="w-4 h-4"/></Button>
                      <Button size="icon" variant="destructive" onClick={async () => {
                        if (!confirm(`Tem certeza que deseja eliminar a Folha de Obra ${job.numero_fo}? Esta ação irá eliminar todos os itens e dados logísticos associados.`)) {
                          return;
                        }
                        ...
                        try {
                          ...
                          setAllItems(prevItems => prevItems.filter(item => item.folha_obra_id !== job.id));
                  ...
                  <TableCell className="text-center p-0 w-[9ch] max-w-[9ch]">
                    <Checkbox
                      checked={!!job.fatura}
                      onCheckedChange={async (checked) => {
                        const value = checked === 'indeterminate' ? false : checked;
                        ...
                        // await supabase.from("folhas_obras").update({ fatura: value }).eq("id", job.id);
                      }}
                    />
                  </TableCell>
                </TableRow>
            ...
            {sorted.length===0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-8">Nenhum trabalho encontrado.</TableCell>
              </TableRow>
            )}
                </TableBody>
              </Table>
      ...
      {hasMoreJobs && !loading.jobs && (
        <div className="flex justify-center mt-4">
          <Button variant="outline" onClick={loadMoreJobs}>
            <Loader2 className={`w-4 h-4 mr-2 ${loading.jobs ? 'animate-spin' : ''}`} />
        ...
        <TabsContent value="concluidos">
          ...
          {loading.jobs && jobs.length === 0 ? (
            ...
            <TableBody>
              ...
              {sorted.length===0 && (
                <TableRow>
                  <TableCell colSpan={12} className="text-center py-8">Nenhum trabalho concluído encontrado.</TableCell>
                </TableRow>
              )}
                  </TableBody>
                  </Table>
      ...
      {/* drawer (single level) */}
      <Drawer open={!!openId} onOpenChange={(o)=>!o && setOpenId(null)} shouldScaleBackground={false}>
        <DrawerContent className="h-[98vh] min-h-[98vh] max-h-[98vh] !top-0 overflow-y-auto !transform-none !filter-none !backdrop-filter-none will-change-auto" style={{ transform: 'none', filter: 'none', backfaceVisibility: 'hidden', perspective: '1000px' }}>
      ...
      <Dialog open={duplicateDialog.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {duplicateDialog.type === 'orc' ? 'ORC Duplicado' : 'FO Duplicada'}
            </DialogTitle>
            <DialogDescription>
          ...
          {duplicateDialog.existingJob && (
            <div className="my-4 p-4 bg-orange-50 border border-orange-200 rounded-md">
              <h4 className="font-semibold text-orange-800 mb-2">Trabalho Existente:</h4>
              <div className="text-sm text-orange-700 space-y-1">
                <div><strong>FO:</strong> {duplicateDialog.existingJob.numero_fo}</div>
                <div><strong>ORC:</strong> {duplicateDialog.existingJob.numero_orc || 'N/A'}</div>
                <div><strong>Campanha:</strong> {duplicateDialog.existingJob.nome_campanha}</div>
                <div><strong>Cliente:</strong> {duplicateDialog.existingJob.cliente || 'N/A'}</div>
              </div>
            </div>
          )}
...
function JobDrawerContent({ jobId, jobs, items, onClose, supabase, setJobs, setAllItems, fetchJobsSaiuStatus }: JobDrawerProps) {
  const job = jobs.find((j) => j.id === jobId);
  if (!job) return null;

  // Simply show what's in items_base for this job
  const jobItems = items.filter((i) => i.folha_obra_id === jobId);
  console.log('🏭 Production items for job', jobId, ':', jobItems);
  console.log('🏭 All items available:', items.length);

  // Sorting state for drawer table
    ...
    arr.sort((a, b) => {
      ...
      switch (sortCol) {
        case 'bulk':
          A = a.id; B = b.id; break;
        case 'descricao':
          A = a.descricao; B = b.descricao; break;
        case 'codigo':
          A = a.codigo || ''; B = b.codigo || ''; break;
        case 'quantidade':
          A = a.quantidade ?? 0; B = b.quantidade ?? 0; break;
        case 'acoes':
          A = a.id; B = b.id; break;
        default:
          A = a.id; B = b.id;
  ...
  // --- Logistica Tab State/Logic (FO-based, always show all items) ---
  ...
  const fetchLogisticaRows = async () => {
    ...
    const itemsWithoutLogistics = jobItems.filter(item => 
      !logisticsData.some(l => l.item_id === item.id)
    );
    ...
    if (itemsWithoutLogistics.length > 0) {
      ...
      const { data: newLogisticsData, error: logisticsInsertError } = await supabase
        .from('logistica_entregas')
        .insert(newLogisticsEntries)
        .select(`
          *,
          items_base!inner (
            id,
    ...
    if (!sourceRowId && mergedRows.length > 0) {
      const firstCompleteRow = mergedRows.find(row => 
        row.local_recolha && row.local_entrega && row.transportadora
      );
      ...
      <div className="mb-6 p-4 uppercase">
        <div className="flex gap-8 items-center mb-2">
          ...
          <div className="flex-1">
            ...
            <div className="font-mono truncate">{job.nome_campanha}</div>
          </div>
        </div>
      </div>
      {/* Tabs below job info */}
      <Tabs defaultValue="producao" className="w-full pl-4" onValueChange={async (value) => {
        if (value === "logistica") {
              ...
              <div className="flex gap-2 items-center">
                ...
                <Button size="sm" variant="outline" onClick={async () => {
                  try {
                    ...
                    const { data: itemsData, error: itemsError } = await supabase
                      .from('designer_items')
                      .select('item_id, paginacao, items_base(folha_obra_id, descricao, codigo, quantidade, brindes)')
                      .eq('items_base.folha_obra_id', job.id);
                    
                    if (!itemsError && itemsData) {
                      const refreshedItems: Item[] = itemsData.map((d: any) => ({
                        id: d.item_id,
                      ...
                      setAllItems(prev => {
                        const filtered = prev.filter(item => item.folha_obra_id !== job.id);
              ...
              <div className="max-h-[40vh] overflow-y-auto w-full">
                <Table className="w-full border-0">
                  <TableHeader>
                    <TableRow className="rounded-none">
                      <TableHead
                        className="sticky top-0 z-10 w-10 text-center bg-[var(--orange)] cursor-pointer select-none uppercase rounded-none border-b-2 border-border"
                        onClick={() => toggleSort('bulk')}
                      >
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span>B {sortCol === 'bulk' && (sortDir === 'asc' ? <ArrowUp className="inline w-3 h-3 ml-1" /> : <ArrowDown className="inline w-3 h-3 ml-1" />)}</span>
                            </TooltipTrigger>
                      ...
                      <TableHead
                        className="sticky top-0 z-10 bg-[var(--orange)] cursor-pointer select-none uppercase rounded-none border-b-2 border-border"
                        onClick={() => toggleSort('descricao')}
                      >
                        Item {sortCol === 'descricao' && (sortDir === 'asc' ? <ArrowUp className="inline w-3 h-3 ml-1" /> : <ArrowDown className="inline w-3 h-3 ml-1" />)}
                      </TableHead>
                      <TableHead
                        className="sticky top-0 z-10 bg-[var(--orange)] cursor-pointer select-none w-72 uppercase rounded-none border-b-2 border-border"
                        onClick={() => toggleSort('codigo')}
                      >
                        Código {sortCol === 'codigo' && (sortDir === 'asc' ? <ArrowUp className="inline w-3 h-3 ml-1" /> : <ArrowDown className="inline w-3 h-3 ml-1" />)}
                      </TableHead>
                      <TableHead
                        className="sticky top-0 z-10 w-24 text-right bg-[var(--orange)] cursor-pointer select-none uppercase rounded-none border-b-2 border-border"
                        onClick={() => toggleSort('quantidade')}
                      >
                        Quantidade {sortCol === 'quantidade' && (sortDir === 'asc' ? <ArrowUp className="inline w-3 h-3 ml-1" /> : <ArrowDown className="inline w-3 h-3 ml-1" />)}
                      </TableHead>

                      <TableHead
                        className="sticky top-0 z-10 text-center w-[120px] bg-[var(--orange)] cursor-pointer select-none uppercase rounded-none border-b-2 border-border"
                        onClick={() => toggleSort('acoes')}
                      >
                        Ações {sortCol === 'acoes' && (sortDir === 'asc' ? <ArrowUp className="inline w-3 h-3 ml-1" /> : <ArrowDown className="inline w-3 h-3 ml-1" />)}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                <TableBody>
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={async (checked) => {
                            ...
                            setAllItems(prevItems => 
                              prevItems.map(item => 
                                item.id === it.id ? { ...item, brindes: value } : item
                              )
                            );
                            // Persist to Supabase
                            await supabase.from('items_base').update({ brindes: value }).eq('id', it.id);
                          }}
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          ...
                          onChange={e => {
                            ...
                            setAllItems(prevItems => 
                              prevItems.map(item => 
                                item.id === it.id ? { ...item, descricao: value } : item
                              )
                            );
                          }}
                          onBlur={async e => {
                            ...
                            // Update items_base
                            await supabase.from('items_base').update({ descricao: newDescricao }).eq('id', it.id);
                            // Also update logistica_entregas with the same description
                            await supabase.from('logistica_entregas').update({ descricao: newDescricao }).eq('item_id', it.id);
                          }}
                      ...
                      <TableCell className="w-72">
                        <Input
                          ...
                          onChange={e => {
                            ...
                            setAllItems(prevItems => 
                              prevItems.map(item => 
                                item.id === it.id ? { ...item, codigo: value } : item
                              )
                            );
                          }}
                          onBlur={async e => {
                            await supabase.from('items_base').update({ codigo: e.target.value }).eq('id', it.id);
                          }}
                          className="rounded-none w-full"
                        />
                      </TableCell>
                      <TableCell className="text-right">
                        <Input
                          ...
                          onChange={e => {
                            ...
                            setAllItems(prevItems => 
                              prevItems.map(item => 
                                item.id === it.id ? { ...item, quantidade: numValue } : item
                              )
                            );
                          }}
                          onBlur={async e => {
                            await supabase.from('items_base').update({ quantidade: e.target.value === '' ? null : Number(e.target.value) }).eq('id', it.id);
                          }}
                          className="rounded-none w-20 text-right"
                        />
                      </TableCell>

                      <TableCell className="flex gap-2 justify-center pr-2 w-[120px]">
                        <Button size="icon" variant="secondary" className="aspect-square size-10 !p-0 flex items-center justify-center" onClick={async () => {
                          ...
                          await supabase.from('designer_items').insert({
                            item_id: newBase.id,
                            em_curso: true,
                            duvidas: false,
                            maquete_enviada: false,
                          ...
                          // Update global state
                          setAllItems(prev => [...prev, duplicatedItem]);
                        ...
                        <Button size="icon" variant="destructive" className="aspect-square size-10 !p-0 flex items-center justify-center" onClick={async () => {
                          // Delete item: remove from logistics, designer_items, then items_base
                          await supabase.from('logistica_entregas').delete().eq('item_id', it.id);
                          await supabase.from('designer_items').delete().eq('item_id', it.id);
                          await supabase.from('items_base').delete().eq('id', it.id);
                          
                          // Remove from global state
                          setAllItems(prev => prev.filter(item => item.id !== it.id));
        ...
        <TabsContent value="logistica">
          <div className="mt-6">
            <div className="flex justify-between items-start mb-6">
              <div className="p-0">
                <h2 className="text-xl font-bold">Listagem Recolhas Entregas</h2>
                <p className="text-sm text-muted-foreground">
              ...
              <div className="flex gap-2 items-center">
                <Button 
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (designerError) {
                        console.error('Error creating designer item:', designerError);
                        alert(`Erro ao criar entrada de design: ${designerError.message}`);
                        setLogisticaLoading(false);
                        return;
                      }
                      ...
                      setProductionItems(prevItems => [...prevItems, newItem]);
                      
                      // 5. Refresh logistica data with updated production items
                      await fetchLogisticaRows([...productionItems, newItem]);
                ...
                <Button size="sm" variant="secondary" onClick={async () => {
                  ...
                  try {
                    ...
                    const quantityMap = new Map(itemsData.map((item: any) => [item.id, item.quantidade]));

                    // Update all logistica_entregas records with original quantities
                    const updatePromises = logisticaRows
                      .filter(row => row.item_id && quantityMap.has(row.item_id))
                      .map(row => {
                ...
                <Button 
                  ...
                  const sourceRecord = logisticaRows.find(r => r.id === sourceRowId);
                  if (!sourceRecord) {
                  ...
                  const deliveryInfo = {
                    ...
                    contacto_entrega: sourceRecord.contacto_entrega || undefined,
                    telefone_entrega: sourceRecord.telefone_entrega || undefined,
                    notas: sourceRecord.notas || undefined, // Include popover notes
                    data: sourceRecord.data || new Date().toISOString().split('T')[0],
                  ...
                  {sourceRowId && (
                    <span className="ml-2 text-xs bg-primary/20 px-2 py-1 rounded">
                      Fonte: {logisticaRows.find(r => r.id === sourceRowId)?.items_base?.descricao || 'Selecionada'}
                    </span>
                  )}
                </Button>
              </div>
                ...
                <div className="max-h-[70vh] overflow-y-auto w-full rounded-none">
                  <LogisticaTableWithCreatable
                      ...
                      onItemSave={async (row: any, value) => {
                        ...
                        if (row.id) {
                          // Update existing logistics record
                          await updateLogisticaField(row.id, "descricao", value, null);
                          setLogisticaRows(prevRows => prevRows.map(r => 
                      ...
                      onConcluidoSave={async (row: any, value) => {
                        if (!row.id && row.item_id) {
                          ...
                          // Update local state instead of refetching
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.id === row.id ? { ...r, concluido: value } : r
                          ));
                        }
                      }}
                      onDataConcluidoSave={async (row: any, value) => {
                        if (!row.id && row.item_id) {
                          ...
                          await updateLogisticaField(row.id, "data_concluido", value, null);
                          // Update local state instead of refetching
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.id === row.id ? { ...r, data_concluido: value } : r
                          ));
                        }
                      }}
                      ...
                      onBrindesSave={async (row, value) => { 
                        if (row.items_base?.id) {
                          ...
                          if (value !== undefined && value !== null) {
                            await updateItemBaseField(row.items_base.id, "brindes", value, null);
                            setLogisticaRows(prevRows => prevRows.map(r => 
                            ...
                            console.warn('Attempted to update items_base.brindes with empty value, skipping.');
                          }
                        }
                      }}
                      onClienteChange={async (row, value) => { 
                        if (row.items_base?.folhas_obras?.id) {
                          await updateFolhaObraField(row.items_base.folhas_obras.id, "id_cliente", value, null);
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.items_base?.folhas_obras?.id === row.items_base?.folhas_obras?.id ? { 
                              ...r, 
                              items_base: { 
                                ...r.items_base, 
                                folhas_obras: { 
                      ...
                      onRecolhaChange={async (rowId, value) => { 
                        if (rowId) {
                          const armazem = logisticaArmazens?.find((a: any) => a.value === value);
                          const armazemName = armazem ? armazem.label : '';
                          await Promise.all([
                            updateLogisticaField(rowId, "id_local_recolha", value, null),
                            updateLogisticaField(rowId, "local_recolha", armazemName, null)
                          ]);
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.id === rowId ? { ...r, id_local_recolha: value, local_recolha: armazemName } : r
                          ));
                        } else {
                      ...
                      onEntregaChange={async (rowId, value) => { 
                        if (rowId) {
                          ...
                          const armazemName = armazem ? armazem.label : '';
                          await Promise.all([
                            updateLogisticaField(rowId, "id_local_entrega", value, null),
                            updateLogisticaField(rowId, "local_entrega", armazemName, null)
                          ]);
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.id === rowId ? { ...r, id_local_entrega: value, local_entrega: armazemName } : r
                          ));
                        } else {
                      ...
                      onTransportadoraChange={async (row: any, value) => { 
                        if (!row.id && row.item_id) {
                          ...
                          await updateLogisticaField(row.id, "transportadora", value, null);
                          // Update local state instead of refetching
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.id === row.id ? { ...r, transportadora: value } : r
                          ));
                        }
                      }}
                      onQuantidadeSave={async (row: any, value) => { 
                        ...
                        const existingEntriesForItem = logisticaRows.filter(r => 
                          (r.item_id || r.items_base?.id) === itemId
                        );
                        ...
                        if (error || !newLogisticsEntry) {
                          ...
                          await fetchLogisticaRows(productionItems);
                          return;
                        }
                        setLogisticaRows(prevRows => [...prevRows, newLogisticsEntry]);
                      }}
                      onNotasSave={async (row: any, outras: string, contacto?: string, telefone?: string, contacto_entrega?: string, telefone_entrega?: string, data?: string | null) => { 
                        if (!row.id && row.item_id) {
                          ...
                          await Promise.all([
                            ...
                            updateLogisticaField(row.id, "contacto", contacto || undefined, null),
                            updateLogisticaField(row.id, "telefone", telefone || undefined, null),
                            updateLogisticaField(row.id, "contacto_entrega", contacto_entrega || undefined, null),
                            updateLogisticaField(row.id, "telefone_entrega", telefone_entrega || undefined, null),
                            updateLogisticaField(row.id, "data", data || row.data, null),
                          ]);
                          // Update local state instead of refetching
                          setLogisticaRows(prevRows => prevRows.map(r => 
                            r.id === row.id ? { 
                      ...
                      onDeleteRow={async (rowId) => { 
                        try {
                          ...
                          const rowToDelete = logisticaRows.find(row => row.id === rowId);
                          if (!rowToDelete) {
                          ...
                          const itemId = rowToDelete.item_id || rowToDelete.items_base?.id;
                          const entriesForSameItem = logisticaRows.filter(row => 
                          ...
                          setLogisticaRows(prevRows => prevRows.filter(row => row.id !== rowId));
                          // Refresh saiu status for this job
                          await fetchJobsSaiuStatus([job.id]);
                        } catch (error) {
